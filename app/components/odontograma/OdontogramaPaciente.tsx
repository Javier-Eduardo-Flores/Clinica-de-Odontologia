...
function getBadgeStyles(estadoObj: any) {
  ...
  if (nombre.includes("ausente") || nombre.includes("extra") || nombre.includes("perdid")) {
    return { bg: "bg-slate-100", text: "text-slate-800", border: "border-slate-200", dot: "bg-slate-700" };
  }
  if (nombre.includes("carie") || nombre.includes("fractura") || nombre.includes("urgente")) {
    return { bg: "bg-red-50", text: "text-red-800", border: "border-red-100", dot: "bg-red-500" };
  }
  if (nombre.includes("trata") || nombre.includes("corona") || nombre.includes("endo") || nombre.includes("obtur") || nombre.includes("resina")) {
    return { bg: "bg-blue-50", text: "text-blue-800", border: "border-blue-100", dot: "bg-blue-500" };
  }
  if (hexColor && hexColor !== "#DCFCE7") {
    return { bg: "bg-amber-50", text: "text-amber-900", border: "border-amber-100", dotStyle: { backgroundColor: hexColor.startsWith("#") ? hexColor : `#${hexColor}` } };
  }
  return { bg: "bg-emerald-50", text: "text-emerald-800", border: "border-emerald-100", dot: "bg-emerald-500" };
}
...
export function OdontogramaPaciente({ dientes }: { dientes: any[] }) {
  ...
  const estadoObj = dienteSeleccionado?.estadoActual;
  const estadoNombre = estadoObj?.nombre ?? null;
  const observacionesReales =
    dienteSeleccionado?.observaciones ??
    dienteSeleccionado?.observacion ??
    dienteSeleccionado?.observacion_reciente ??
    dienteSeleccionado?.notas ??
    dienteSeleccionado?.diagnostico ??
    dienteSeleccionado?.detalle ??
    null;
          ...
          <div className="space-y-5">
            <div>
              ...
              <p className="text-base font-sans font-bold text-gray-900 leading-snug">
                {dienteSeleccionado.numero_fdi ? `${dienteSeleccionado.numero_fdi} — ` : ""}
                {dienteSeleccionado.nombre || "Sin nombre registrado"}
              </p>
            </div>
            ...
            <div>
              ...
              <span className={`inline-flex items-center gap-2 text-xs font-sans font-semibold px-3 py-1.5 rounded-full border ${badgeStyle.bg} ${badgeStyle.text} ${badgeStyle.border}`}>
                <span className={`w-2 h-2 rounded-full shrink-0 ${badgeStyle.dot || ""}`} style={badgeStyle.dotStyle} />
