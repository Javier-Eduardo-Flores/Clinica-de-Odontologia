...
export default async function JornadasPage({
  ...
  return (
    <div className="flex min-h-screen bg-gray-50">
      <Sidebar activePath="/dashboard/jornadas" />
      ...
      <div className="flex-1">
        <header className="bg-white border-b border-gray-200 px-8 py-4 flex items-center gap-4">
          ...
          <form action="/dashboard/jornadas" method="GET" className="relative flex-1 max-w-xl">
        ...
        <main className="p-8">
          <div className="flex justify-between items-center mb-6">
            <h1 className="text-3xl font-sans font-bold text-gray-900">Jornadas</h1>
            <Link
              href="/dashboard/jornadas/nuevo"
          ...
          {!jornadas || jornadas.length === 0 ? (
            <p className="text-gray-400 font-sans">No se encontraron jornadas.</p>
          ) : (
              ...
              <table className="w-full text-left">
                <thead>
                  <tr className="text-xs font-sans font-bold text-gray-400 uppercase border-b border-gray-100">
                    ...
                    <th className="py-4 px-6">Hora de Inicio</th>
                    <th className="py-4 px-6">Hora de Fin</th>
                    <th className="py-4 px-6 text-right">Acciones</th>
                  </tr>
                </thead>
                <tbody>
                  {jornadas.map((j) => (
                    <tr key={j.id_jornada} className="border-b border-gray-50 last:border-0 hover:bg-gray-50">
                      <td className="py-4 px-6 font-sans font-semibold text-gray-900">{j.nombre}</td>
                      <td className="py-4 px-6 text-sm font-sans text-gray-500">{formatearHora(j.hora_inicio)}</td>
                      <td className="py-4 px-6 text-sm font-sans text-gray-500">{formatearHora(j.hora_fin)}</td>
                      <td className="py-4 px-6 text-right">
                        <div className="flex items-center justify-end gap-2">
                          <Link
                            href={`/dashboard/jornadas/${j.id_jornada}/editar`}
                          ...
                          <Link
                            href={`/dashboard/jornadas/${j.id_jornada}/eliminar`}
