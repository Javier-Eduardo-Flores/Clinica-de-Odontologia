...
import RelojEnVivo from "@/app/components/relojenvivo";
...
export default async function DashboardPage() {
  ...
  // admin, doctor y recepcionista comparten el dashboard operativo/odontólogo
  return <DashboardOdontologo supabase={supabase} perfil={perfil} userId={user.id} />;
}
...
async function DashboardOdontologo({
  ...
  const nombreCompleto = `${perfil.nombre} ${perfil.apellido}`;
  const esStaffAdministrativo = perfil.rol === "admin" || perfil.rol === "recepcionista";
  ...
  // INGRESOS MENSUALES — solo relevante para admin/recepcionista
  let ingresosMensuales = 0;
  if (esStaffAdministrativo) {
    const primerDiaMes = new Date();
    primerDiaMes.setDate(1);
    primerDiaMes.setHours(0, 0, 0, 0);
    ...
    ingresosMensuales = facturasDelMes?.reduce((suma, f) => suma + Number(f.total), 0) ?? 0;
  }
  ...
  // PERSONAL — solo visible para admin/recepcionista
  const { data: personal } = esStaffAdministrativo
    ? await supabase.from("profiles").select("nombre, apellido, rol").limit(3)
    : { data: null as { nombre: string; apellido: string; rol: string }[] | null };

  // PACIENTES RECIENTES — solo visible para admin/recepcionista
  const { data: facturasRecientes } = esStaffAdministrativo
    ? await supabase
        .from("factura")
        .select("id_factura, fecha, total, estado, pacientes(primer_nombre, primer_apellido)")
        .order("fecha", { ascending: false })
  ...
  // CITAS PENDIENTES DE CONFIRMAR
  // Un doctor solo debe ver (y poder confirmar) las citas que tiene asignadas.
  let citasPendientesQuery = supabase
    .from("citas")
    .select("id_cita, fecha_cita, motivo, id_odontologo, pacientes(primer_nombre, primer_apellido)")
    .eq("estado", 1)
    .order("fecha_cita", { ascending: true })
    .limit(5);

  if (perfil.rol === "doctor") {
    citasPendientesQuery = citasPendientesQuery.eq("id_odontologo", userId);
  }

  const { data: citasPendientes } = await citasPendientesQuery;
    ...
    <div className="flex min-h-screen bg-gray-50">
      ...
      <div className="flex-1">
        <header className="bg-white border-b border-gray-200 px-8 py-4 flex items-center gap-4">
          <form action="/dashboard/pacientes" method="GET" className="relative flex-1 max-w-xl">
            ...
            <input
              type="text"
              name="q"
              placeholder={perfil.rol === "doctor" ? "Buscar pacientes..." : "Buscar pacientes, registros o personal..."}
              className="w-full border border-gray-300 rounded-lg py-2 pl-10 pr-3 font-sans focus:outline-none focus:ring-2 focus:ring-clinica-dark"
            />
          </form>
          {(perfil.rol === "admin" || perfil.rol === "recepcionista") && (
            <Link
              href="/dashboard/citas/agendar"
              className="bg-clinica-dark text-white font-sans font-semibold px-4 py-2 rounded-lg hover:bg-clinica-medium"
            >
              + Agendar Cita
            </Link>
        ...
        <main className="p-8">
          ...
          <div className={`grid gap-4 mb-8 ${esStaffAdministrativo ? "grid-cols-3" : "grid-cols-2"}`}>
            <StatCard
              icon={User} iconColor="text-clinica-dark" iconBg="bg-blue-50" borderColor="border-clinica-dark"
              badge="" badgeColor=""
              value={String(totalPacientes ?? 0)}
          ...
          <div className={`grid gap-6 mb-8 ${esStaffAdministrativo ? "grid-cols-3" : "grid-cols-1"}`}>
            <div className={`${esStaffAdministrativo ? "col-span-2" : ""} bg-white rounded-xl shadow-sm p-6`}>
              <div className="flex justify-between items-center mb-4">
                <h2 className="text-xl font-sans font-bold text-gray-900">Gestión de Tratamientos</h2>
                <Link href="/dashboard/tratamientos" className="text-sm font-sans font-semibold text-clinica-dark hover:underline">
                  Ver todos
            ...
            <div className="bg-white rounded-xl shadow-sm p-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                {facturasRecientes?.map((f) => (
                  <Link
                    key={f.id_factura}
                    href={`/dashboard/facturacion?factura=${f.id_factura}`}
                    className="border border-gray-100 rounded-lg p-4 hover:border-clinica-dark transition-colors"
                  >
                    <div className="flex items-start gap-3">
                      <div className="w-10 h-10 rounded-full bg-clinica-light flex items-center justify-center font-sans font-bold text-clinica-dark text-sm shrink-0">
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-6xl mx-auto">
        ...
        <main className="p-8">
          ...
          {/* Odontograma del paciente con layout de 2 columnas igual al del doctor */}
          <div id="odontograma" className="mb-8 scroll-mt-8">
            <div className="mb-4">
              <h2 className="text-xl font-sans font-bold text-gray-900">Mi Odontograma</h2>
